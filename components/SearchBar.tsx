...
export function ProjectSearch() {
    ...
    return (
        <div className=' h-auto p-3 mr-6 max-sm:bg-gray-400 max-sm:mr-0'>
            <div className=" ">
                <div className="flex items-center justify-between gap-2 h-8 w-63">
                    <input
                    aria-label='search meetings'
                        type="search"
                        placeholder="Search speaker, leader or meeting type..."
                        defaultValue={searchParams.get('query')?.toString()}
                        onChange={(e) => handleSearch(e.target.value)}
                        className="w-full h-full text-sm text-black rounded-md ring-1 focus:ring-2 bg-white outline-none px-2 py-1"
                    />
